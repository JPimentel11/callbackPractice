...
function contains(namesArr, checkName, cb) {
  var isHeThere = false;
  for (i = 0; i < namesArr.length; i++) {
    if (checkName === namesArr[i]) {
      isHeThere = true;
    }
  }
  cb(isHeThere);
...
function uniq(namesArr, cb) {
  uniqueArray = namesArr.filter(function(item, pos) {
...
function getUserById(usersArr, userId, cbFunc) {
  var selectedUser;
  for (var i = 0; i < usersArr.length; i++) {
    if (usersArr[i].id === userId) {
      selectedUser = usersArr[i];
    }
  }
  cbFunc(selectedUser);
}
